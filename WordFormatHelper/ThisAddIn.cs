...
public sealed class ThisAddIn : AddInBase
{
	...
	public struct TextFormatSet
	{
		...
		internal bool PunctuationSpace;
	...
	internal void SetTableFormat(Table tableToSet, [Optional] bool firstRowBold, [Optional] bool firstColumnBold, [Optional] bool setInnerMargin, [Optional] int innerMarginType, [Optional] float innerMargin, [Optional] bool removeUselessLine, [Optional] bool setBorder, [Optional] int borderType, [Optional] int borderLineWidth)
	{
		...
		foreach (Cell cell in tableToSet.Range.Cells)
		{
			...
			if (removeUselessLine && (Regex.IsMatch(cell.Range.Text, "^[\\r ]{1,}") || Regex.IsMatch(cell.Range.Text, "[\\r ]{1,}[\\a]$")))
			{
				cell.Range.Text = Regex.Replace(Regex.Replace(cell.Range.Text, "^[\\r ]{1,}", ""), "[\\r ]{1,}[\\a]$", "");
			}
		}
	}
	...
	internal void CreateNewTable(bool ThreeLine, bool ThreeLineExtra, bool BroadOuterLine, bool TitleRowFilled, bool SummaryRow, bool SummaryColumn, bool SummaryRowFilled, bool Diagonal)
	{
		...
		else
		{
			...
			if (Diagonal)
			{
				if (tableSettings.FixRowHeight)
				{
					float num2 = (float)Math.Round((0.0023f * tableSettings.FontSize + 1.38f) * tableSettings.FontSize);
					table.Range.ParagraphFormat.LineSpacingRule = WdLineSpacing.wdLineSpaceExactly;
					table.Range.ParagraphFormat.LineSpacing = num2;
					table.Rows[1].Height = num2 * 2f;
				}
				else
				{
	...
	internal void PunctuationSwitch(Range exRange, List<char> charSetA, List<char> charSetB)
	{
		if (exRange.Start != exRange.End)
		{
			for (int i = 0; i < charSetA.Count; i++)
			{
				Find find = exRange.Find;
				object FindText = charSetA[i].ToString();
				object obj = charSetB[i].ToString();
	...
	internal List<char> GetEngCharSet()
	{
		List<char> list = new List<char>();
		if (textFormatSet.SetA)
		{
