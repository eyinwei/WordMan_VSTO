...
public class WordFormatHelperRibbon : RibbonBase
{
	...
	private void AddTips()
	{
		foreach (RibbonGroup group in Tab_WordFormatAssistant.Groups)
		{
			...
			{
				if (Resources.ResourceManager.GetString(item.Name) != null)
				{
					item.GetType().GetProperty("SuperTip")?.SetValue(item, Resources.ResourceManager.GetString(item.Name));
				}
	...
	private void Application_WindowActivate(Document Doc, Window Wn)
	{
		...
		{
			(hwndPaneDic[Wn.Hwnd].Control as Settings).ReadSettings();
		}
	}
	...
	private void Btn_PunctuationEng2Chn_Click(object sender, RibbonControlEventArgs e)
	{
		...
		try
		{
			...
			{
				Globals.ThisAddIn.PunctuationSwitch(item, Globals.ThisAddIn.GetEngCharSet(), Globals.ThisAddIn.GetChnCharSet());
			}
		}
		finally
		{
	...
	private void Btn_PunctuationChn2Eng_Click(object sender, RibbonControlEventArgs e)
	{
		...
		try
		{
			...
			foreach (Range item in multiSelection)
			{
				Globals.ThisAddIn.PunctuationSwitch(item, Globals.ThisAddIn.GetChnCharSet(), Globals.ThisAddIn.GetEngCharSet());
			}
		}
		finally
		{
	...
	private void Btn_DeleteSpace_Click(object sender, RibbonControlEventArgs e)
	{
		...
		try
		{
			...
			foreach (Range item in multiSelection)
			{
				...
				{
					MatchCollection matchCollection = Regex.Matches(item.Text, "([\\u0021\\u0022\\u002C\\u002E\\u003A\\u003B\\u003F\\u3001\\u3002\\uFF01\\uFF0C\\uFF1A\\uFF1B\\uFF1F])[ ]{1,}");
					for (int i = 0; i < matchCollection.Count; i++)
					{
						Find find = item.Find;
						object FindText = matchCollection[i].Value;
						object obj = matchCollection[i].Value.TrimEnd();
				...
				else
				{
					...
					object MatchSoundsLike = WdReplace.wdReplaceAll;
					object MatchWildcards = Type.Missing;
					object MatchWholeWord = Type.Missing;
					object MatchCase = Type.Missing;
	...
	private void Ga_InsertDate_Click(object sender, RibbonControlEventArgs e)
	{
		...
		case 0:
		{
			Globals.ThisAddIn.Application.Selection.InsertAfter(DateTime.Today.Year + "年" + DateTime.Today.Month + "月" + DateTime.Today.Day + "日");
			Selection selection16 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection16.Collapse(ref Count);
			break;
		}
		case 1:
		{
			Globals.ThisAddIn.Application.Selection.InsertAfter(DateTime.Today.Year + "年");
			Selection selection11 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection11.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("    ");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineSingle;
			Selection selection12 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection12.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("月");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineNone;
			Selection selection13 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection13.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("    ");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineSingle;
			Selection selection14 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection14.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("日");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineNone;
			Selection selection15 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection15.Collapse(ref Count);
			break;
		}
		case 2:
		{
			Globals.ThisAddIn.Application.Selection.InsertAfter(DateTime.Today.Year + "年" + DateTime.Today.Month + "月");
			Selection selection8 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection8.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("    ");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineSingle;
			Selection selection9 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection9.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("日");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineNone;
			Selection selection10 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection10.Collapse(ref Count);
			break;
		}
		case 3:
		{
			...
			Count = WdCollapseDirection.wdCollapseEnd;
			selection.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("        ");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineSingle;
			Selection selection2 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection2.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("年");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineNone;
			Selection selection3 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection3.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("    ");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineSingle;
			Selection selection4 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection4.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("月");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineNone;
			Selection selection5 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection5.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("    ");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineSingle;
			Selection selection6 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection6.Collapse(ref Count);
			Globals.ThisAddIn.Application.Selection.InsertAfter("日");
			Globals.ThisAddIn.Application.Selection.Font.Underline = WdUnderline.wdUnderlineNone;
			Selection selection7 = Globals.ThisAddIn.Application.Selection;
			Count = WdCollapseDirection.wdCollapseEnd;
			selection7.Collapse(ref Count);
			break;
		}
		}
	}
	...
	private void Ga_Bracketed_Click(object sender, RibbonControlEventArgs e)
	{
		...
		if (ThisAddIn.textFormatSet.FullWidthBracket)
		{
			array[2] = "（";
			array[3] = "［";
			array[4] = "｛";
		...
		foreach (Range item in multiSelection)
		{
			if (ThisAddIn.textFormatSet.RemoveBrackets && Regex.IsMatch(item.Text, "^[“《（［｛＜\\(\\[\\{\\<〔]{1}.*[”》）］｝＞\\)\\]\\}\\>〕]$"))
			{
				Document activeDocument = Globals.ThisAddIn.Application.ActiveDocument;
				object Start = item.Start;
				object End = item.Start + 1;
				Range range = activeDocument.Range(ref Start, ref End);
				End = Type.Missing;
				Start = Type.Missing;
				range.Delete(ref End, ref Start);
				Document activeDocument2 = Globals.ThisAddIn.Application.ActiveDocument;
				Start = item.End - 1;
				End = item.End;
				Range range2 = activeDocument2.Range(ref Start, ref End);
				End = Type.Missing;
				Start = Type.Missing;
				range2.Delete(ref End, ref Start);
			}
			item.InsertBefore(array[Ga_Bracketed.SelectedItemIndex]);
			item.InsertAfter(array2[Ga_Bracketed.SelectedItemIndex]);
		}
	}
	...
	private void ParagrahIndetSet_ButtonClick(object sender, RibbonControlEventArgs e)
	{
		...
		foreach (Paragraph paragraph in Globals.ThisAddIn.Application.Selection.Range.Paragraphs)
		{
			if (text == "Btn_ParagrahIndent2Char")
			{
				paragraph.Format.LeftIndent = 0f;
				paragraph.Format.FirstLineIndent = 0f;
				paragraph.Format.IndentFirstLineCharWidth(2);
			}
			else if (text == "Btn_ParagrahNoIndent")
			{
				paragraph.Format.LeftIndent = 0f;
				paragraph.Format.FirstLineIndent = 0f;
			}
	...
	private void Btn_DeleteBlankLine_Click(object sender, RibbonControlEventArgs e)
	{
		...
		foreach (Paragraph paragraph in Globals.ThisAddIn.Application.Selection.Paragraphs)
		{
			if (Regex.IsMatch(paragraph.Range.Text, "^\\s*\\r$"))
			{
				list.Add(paragraph);
			}
		}
	...
	public void ApplyFixFormatPainter()
	{
		...
		FixFormatPainterSetting.FixFormat fixFormat = ThisAddIn.formatPainter.StoredFormat[ThisAddIn.formatPainter.CurrentID];
		foreach (Range item in multiSelection)
		{
			item.Font.Name = fixFormat.EngFontName;
			item.Font.NameFarEast = fixFormat.ChnFontName;
			item.Font.Size = fixFormat.FontSize;
			item.Font.Bold = (fixFormat.Bold ? (-1) : 0);
			item.Font.Italic = (fixFormat.Italic ? (-1) : 0);
			item.Font.Underline = (fixFormat.Underline ? WdUnderline.wdUnderlineSingle : WdUnderline.wdUnderlineNone);
			if (fixFormat.UseColor)
			{
				Color color = Color.FromArgb(fixFormat.TextColor);
				item.Font.Color = (WdColor)ThisAddIn.RGB(color.R, color.G, color.B);
			}
			if (fixFormat.Shading)
			{
				Color color2 = Color.FromArgb(fixFormat.ShadingColor);
				item.Shading.Texture = WdTextureIndex.wdTextureSolid;
				item.Shading.ForegroundPatternColor = (WdColor)ThisAddIn.RGB(color2.R, color2.G, color2.B);
			}
		}
	}
	...
	private void InitializeComponent()
	{
		...
		Ga_Bracketed.ColumnCount = 1;
		Ga_Bracketed.Image = Resources.Brackets;
		Ga_Bracketed.ItemImageSize = new Size(150, 30);
		...
		ribbonDropDownItem20.Label = "小括号";
		...
		ribbonDropDownItem22.Label = "大括号";
		...
		ribbonDropDownItem24.Label = "方括号2";
